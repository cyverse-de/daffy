...
def _parse_args(args: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--service", help="service name (or SERVICE_NAME)")
    parser.add_argument("--local-db", help="local DuckDB buffer path (or DAFFY_LOCAL_DB)")
    ...
    parser.add_argument("--scrooge-uri", help="Scrooge quack URI, e.g. quack:host:9494 (or SCROOGE_URI)")
    parser.add_argument("--scrooge-token", help="Scrooge auth token (prefer SCROOGE_TOKEN)")
    parser.add_argument("--flush-bytes", type=int, help="flush when buffered bytes exceed this (or DAFFY_FLUSH_BYTES)")
    parser.add_argument("--flush-interval", type=float, help="max seconds between flushes (or DAFFY_FLUSH_INTERVAL)")
    parser.add_argument("--max-buffer-bytes", type=int, help="drop oldest beyond this when Scrooge is down (or DAFFY_MAX_BUFFER_BYTES)")
